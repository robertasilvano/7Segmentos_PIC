...
;;;;;;;;;;;;
;    sempre incrementa a unidade
;    antes de incrementar, verifica se ela eh nove
;	se sim, tem que usar o clear unidade
;	    verifica a dezena
;		se for nove, clear dezena
;		se nao, so incrementa
;	se não, so incrementa a unidade
...
incrementa:
    ...
    movlw 0x09 ; coloca o valor imediato 10h no acumulador
    
    subwf unidade, W ; subtrai 10h da unidade, se a conta der 0, unidade=10, z(STATUS, 2)=1
    btfsc STATUS, 2 ; se z=0 na conta anterior, significa que a unidade ainda não chegou no 10, então vai incrementar a unidade. Precisa disso se não ele vai pular valores que tem unidade 0. Por exemplo: do 9 iria direto pro 11.
...
clear_unidade:
    ...
    call f_inc_dezena ; quando precisar zerar a unidade, significa que chegou em 9 e portanto tem que incrementar a dezena
    call mostrar
    goto loop
